...
function Add_Account() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`${apiLinks}/AccountType.php`);
        const apiData = await response.json();
        setAccountType(apiData);
        if (apiData.length > 0) {
          setSelectedAccountId(apiData[0].typ_cod);
        }
      } catch (error) {
        console.error(error);
      }
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      formData.append("code", selectedStatus);

      axios
        .post(`${apiLinks}/AddAccount.php`, formData, {
                      ...
                      >
                        ...
                        >
                          ...
                          {getAccountType.map((item) => (
                            <option key={item.typ_cod} value={item.typ_cod}>
                              {item.typ_dsc}
                            </option>
                          ))}
                        </Form.Control>
